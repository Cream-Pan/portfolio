...
export const projects: Record<string, ProjectDetail> = {
  ...
  "maze-game": {
    ...
    description: [
      ...
      "技術的な核心として，『穴掘り法』アルゴリズムに基づいた迷路の自動生成ロジックを独自に実装した．全領域を壁として初期化した後，奇数座標を起点に再帰的な探索を行うことで，必ずゴールに到達可能な迷路を動的に生成する仕組みを採用した．",
      "ユーザーのスキルに応じた柔軟な難易度設定を実現するため，ゲーム開始前に『移動速度』および『制限時間』を選択可能なシステムを導入した．",
  ...
  "typing-game": {
    title: "タイピングゲーム",
    description: [
      "Unityを用いて開発した，PC向けのローマ字タイピング練習アプリケーションである．初学者から上級者まで幅広いユーザーに楽しんでもらうため，3段階の難易度設定および視覚的なフィードバック機能を実装した．",
      "技術的な核心として，外部CSVファイルからの動的な問題読み込みシステムを構築した．System.IOを用いたファイルストリーム処理により，プログラム本体を修正することなく問題データの追加・変更を可能にし，拡張性を実現した．また，入力判定においては，『し』を『shi』または『si』と入力する場合など，複数のローマ字表記パターンを動的に生成・照合するアルゴリズムを実装し，ユーザーの入力習慣を妨げない柔軟な判定ロジックを確立した．",
  ...
  "sichi2025": {
    ...
    description: [
      ...
      "生体センサ（EEG）を用いて，ユーザーが意図的に集中を解いている状態をリアルタイムで検出し，ゲームの難易度にフィードバックする手法を構築した．",
      "本作の核心は，『ダーツを命中させるには集中が必要だが，集中するほど的が収縮し難易度が上昇する』という逆説的な設計にある．",
      "一方で，意図的に集中を解く（非集中状態を作る）ことで的が拡大し有利になるが，肝心の投擲時には再び集中を取り戻さなければならないという，精神状態の高度なコントロールを要するジレンマ体験を実現した．",
      "本作品においては，EEGからのデータ取得・解析を行うシステムに加え，その状態を可視化しゲーム体験へと統合する一連のパイプラインを開発した．"
    ],
  ...
  "sichi2024": {
    ...
    description: [
      "ヒアラブルデバイスの音響特性を活用した新しい入力デバイスの提案である．SICHI 2024へは，１年早く研究室で活動ができるラボワーク制度を使用して参加し，最優秀賞を受賞した，",
      "耳音響認証技術を応用し，耳穴内部の微細な形状の差異を音の反響特性によって識別する手法を開発した．これにより，個人認証と同時に物理的な穴形状を鍵（キー）としたデバイス操作を可能にしている．",
      "動画では異なる形状の穴を用いた選択式クイズや物理ダイヤル金庫の解錠など，物理世界とデジタル情報を繋ぐアプリケーションの実装例を示している．"
    ],
